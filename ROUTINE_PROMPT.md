...
1. **mainブランチで直接作業する**。新しいブランチを作成しないこと。
2. **PRを作成しない**。記事はmainブランチに直接コミット＆pushする。
3. コミットメッセージの形式：
   ```
   content: add news articles for YYYY-MM-DD
   ```
4. コミット後、`git push origin main` で即時反映する。

⚠️ ブランチ作成やPull Requestの作成は絶対に行わないでください。このリポジトリはmainブランチへの直接pushで運用しています。
